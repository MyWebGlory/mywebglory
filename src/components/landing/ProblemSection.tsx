...
const ProblemSection = () => {
  ...
  return <section className="py-24 relative" id="problem">
      ...
      <div className="container relative z-10 px-4 md:px-6" ref={ref}>
        <div className={`text-center mb-16 ${isVisible ? "animate-fade-in" : "opacity-0"}`}>
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">They attract the wrong people, or no one converts.</p>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {problems.map((problem, i) => <div key={i} className={`relative p-6 rounded-2xl bg-card border border-destructive/20 hover:border-destructive/40 transition-all duration-300 ${isVisible ? "animate-fade-in" : "opacity-0"}`} style={{
