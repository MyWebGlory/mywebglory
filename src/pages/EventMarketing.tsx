...
const FinalCTA = () => {
  ...
  // Portal ring expansions - subtle de-zoom
  const ring1Scale = useTransform(scrollYProgress, [0, 0.08], [1.8, 1]);
  const ring2Scale = useTransform(scrollYProgress, [0, 0.1], [2, 1]);
  const ring3Scale = useTransform(scrollYProgress, [0, 0.12], [2.2, 1]);
  const ringOpacity = useTransform(scrollYProgress, [0, 0.06], [0, 1]);

  // Glow intensity - subtle de-zoom
  const glowScale = useTransform(scrollYProgress, [0, 0.1], [2.5, 1]);
  const glowOpacity = useTransform(scrollYProgress, [0, 0.08], [0, 0.5]);

  // Title reveal - gentle de-zoom from front
  const titleOpacity = useTransform(scrollYProgress, [0.08, 0.16], [0, 1]);
  const titleScale = useTransform(scrollYProgress, [0.08, 0.16], [1.6, 1]);

  // Subtitle reveal - de-zoom
  const subtitleOpacity = useTransform(scrollYProgress, [0.16, 0.24], [0, 1]);
  const subtitleScale = useTransform(scrollYProgress, [0.16, 0.24], [1.5, 1]);

  // Growth lever text - de-zoom
  const leverOpacity = useTransform(scrollYProgress, [0.24, 0.32], [0, 1]);
  const leverScale = useTransform(scrollYProgress, [0.24, 0.32], [1.6, 1]);

  // Badges intro text
  const badgeIntroOpacity = useTransform(scrollYProgress, [0.32, 0.40], [0, 1]);
  const badgeIntroScale = useTransform(scrollYProgress, [0.32, 0.40], [1.4, 1]);

  // Badges reveal - staggered de-zoom
  const badge1Opacity = useTransform(scrollYProgress, [0.40, 0.48], [0, 1]);
  const badge1Scale = useTransform(scrollYProgress, [0.40, 0.48], [1.5, 1]);
  const badge2Opacity = useTransform(scrollYProgress, [0.48, 0.56], [0, 1]);
  const badge2Scale = useTransform(scrollYProgress, [0.48, 0.56], [1.5, 1]);
  const badge3Opacity = useTransform(scrollYProgress, [0.56, 0.64], [0, 1]);
  const badge3Scale = useTransform(scrollYProgress, [0.56, 0.64], [1.5, 1]);

  // Final text - de-zoom
  const finalTextOpacity = useTransform(scrollYProgress, [0.64, 0.74], [0, 1]);
  const finalTextScale = useTransform(scrollYProgress, [0.64, 0.74], [1.4, 1]);

  // CTAs - de-zoom
  const ctaOpacity = useTransform(scrollYProgress, [0.74, 0.85], [0, 1]);
  const ctaScale = useTransform(scrollYProgress, [0.74, 0.85], [1.3, 1]);
